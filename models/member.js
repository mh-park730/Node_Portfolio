...
const member = {
	...
	*/
	join : async function() {
		try {
			...
			const sql = `INSERT INTO member (memId, memPw, pwHint, memNm, email, cellPhone)
							VALUES (:memId, :memPw, :pwHint, :memNm, :email, :cellPhone)`;
	...
	*/
	findId : async function(memNm, cellPhone) {
		try {
			if (!memNm || !cellPhone) {
				throw new Error('회원명, 휴대폰번호를 입력해 주세요.');
			}
	...
	*/
	changePw : async function (memNo, memPw) {
		try {
			if (!memNo || !memPw) {
				throw new Error('회원번호, 변경할 비밀번호를 입력해 주세요.');
			}
